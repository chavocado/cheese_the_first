...
router.post('/', function (req, res) {
  ...
  pg.connect(connectionString, function (err, client, done) {
    ...
    client.query( 'INSERT INTO customers (full_name, phone, email, address, address2,city) ' +
                  'VALUES ($1, $2, $3, $4, $5, $6) returning id;',
                   [order.full_name, order.phone, order.email, order.address, order.address2, order.city],
                 function (err, result) {
                   if (err) {
                     res.sendStatus(500);
                     return;
                   ...
                   client.query('INSERT INTO orders (customer_id, payment_type, total, gc1, gc2, gc3, gc4, special_req, status)' +
                                'VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)',
                                [result.rows[0].id, order.payment, order.gcTotal, order.gc1, order.gc2, order.gc3, order.gc4, order.specialReq, 'Recieved'],
                              function (err, result){
                                done();
                                if (err) {
                                  res.sendStatus(500);
